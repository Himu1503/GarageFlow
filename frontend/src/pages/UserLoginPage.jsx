  ...
  return (
    <div className="min-h-screen bg-[#f8fbf1] text-slate-900 flex items-center justify-center p-6">
      <form onSubmit={onSubmit} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
        ...
        <p className="text-slate-600">Login as Admin, Manager, or Staff.</p>
        ...
        <p className="text-sm text-slate-600">
          Customer? <Link to="/login/customer" className="text-[#5f8f1a] underline">Use customer login</Link>
        </p>
        <p className="text-sm text-slate-600">
          New user? <Link to="/register/user" className="text-[#5f8f1a] underline">Create user account</Link>
        </p>
      </form>
    </div>
  );
