  ...
  return (
    <div className="min-h-screen bg-[#f8fbf1] text-slate-900 flex items-center justify-center p-6">
      <form onSubmit={onSubmit} className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm">
        ...
        <p className="text-slate-600">Create a customer account.</p>
        ...
        <input value={form.phone} onChange={(e) => update("phone", e.target.value)} placeholder="Phone" className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2" />
        ...
        <p className="text-sm text-slate-600">
          Already have account? <Link to="/login/customer" className="text-[#5f8f1a] underline">Customer login</Link>
        </p>
      </form>
    </div>
  );
